...
function showSignin(isRefreshAfterSignIn) {
    var modal = UIkit.modal('#modal-signin', {
        bgclose: false,
        center: true
    });
    if (isRefreshAfterSignIn !== undefined) {
        window.refreshAfterSignIn = isRefreshAfterSignIn;
    }
    modal.show();
}
...
function html2md(h) {
    var cleanSpanStart = function (s) {
        var c1 = s.indexOf('<span ');
        if (c1 === -1) {
            return s;
        }
        var c2 = s.indexOf('>', c1);
        ...
        return s.substring(0, c1) + s.substring(c2 + 1);
    };
    var cleanSpanEnds = function (s) {
        var c1 = s.indexOf('</span>');
        if (c1 === -1) {
            return s;
        }
        return s.substring(0, c1) + s.substring(c1 + 7);
    };
...
function buildPagination(page, url) {
    if (page.totalItems === 0) {
        return '<ul class="uk-pagination"><li><span>暂时还没有内容</span></li></ul>';
    }
    var
        i, p,
        start = page.pageIndex - 3,
        end = page.pageIndex + 3,
        L = [],
        addPage = function (n) {
            return '<li><a href="' + url.replace('$', '' + n) + '">' + n + '</a></li>'
        };
    L.push('<ul class="uk-pagination">');
    if (start < 1) {
        start = 1;
    }
    ...
    if (start >= 3) {
        L.push('<li class="uk-disabled"><span>...</span></li>');
    }
    for (p = start; p <= end; p++) {
        if (p === page.pageIndex) {
            L.push('<li class="uk-active"><span>' + p + '</span></li>');
        }
        else {
            L.push(addPage(p));
        }
    ...
    if (end < page.totalPages) {
        L.push(addPage(page.totalPages));
    }
    L.push('</ul>');
    console.log('>>>Page>>>');
    console.log(L.join(''));
...
function createPagination(dom, page) {
    $(dom).html(buildPagination(page, 'javascript:gotoPage($)'));
}
